...
function PrototypeRouter({ onSignOut, initialSurface }: { onSignOut?: () => void; initialSurface: InitialSurface }) {
  ...
  if (surface === "entry") return <EntryScreen onStudent={() => setSurface("pathway")} onAdvisor={() => { dispatch({ type: "LOAD_SCENARIO", scenario: "advisor" }); setSurface("advisor"); }} onPi={() => setSurface("pi")} onAdmin={() => { dispatch({ type: "LOAD_SCENARIO", scenario: "admin" }); setSurface("admin"); }} onSignOut={onSignOut} />;
  if (surface === "workspace") return <FeatureWorkspaces initial={workspace} quick={quick} onBack={() => setSurface(workspaceReturn)} />;
  if (surface === "advisor" || surface === "pi" || surface === "admin") return <ReviewerWorkspace mode={surface} onBack={() => setSurface("entry")} onOpenWorkspace={(next) => openWorkspace(next, false, surface)} />;
...
export function PrototypeShell({ onSignOut, initialSurface = "pathway" }: { onSignOut?: () => void; initialSurface?: InitialSurface } = {}) {
  return <PrototypeProvider><PrototypeRouter onSignOut={onSignOut} initialSurface={initialSurface} /></PrototypeProvider>;
}
