...
export function SignIn({ supabase }: { supabase: SupabaseClient }) {
  ...
  return <main className="production-auth"><section className="production-auth-card compass-signin"><div className="compass-signin__identity"><div className="compass-signin__emblem" aria-hidden="true"><img src={assetUrl("/assets/brand/compass-emblem-v2.png")} alt="" /></div><div className="compass-signin__hero-copy"><p className="compass-signin__eyebrow">Roseman University student support</p><h1>Compass</h1><p className="compass-signin__services">Advising <span>·</span> Tutoring <span>·</span> Events</p><p className="compass-signin__introduction">Sign in once to reach your assigned support and workspaces.</p></div></div><div className="sso-coming-soon" role="note"><strong>Roseman Single Sign On</strong><small>Your Roseman identity and the approved Compass roster are separate access checks.</small></div><div className="production-form"><button type="button" className="primary-button" onClick={() => void signIn()} disabled={busy}>{busy ? "Opening Roseman sign-in…" : "Continue with Roseman Single Sign On"}</button><p className="form-message" aria-live="polite">{message}</p></div><div className="pathway-invite-note"><p>Prematriculating Student Access is Being Developed for Navigate the Pathway Students</p></div><a className="preview-entry" href="/app?preview=creator"><span><strong>Explore Compass demos</strong><small>Choose a fictional, role-scoped workspace to explore.</small></span><span aria-hidden="true">→</span></a><p className="privacy-note">Signing in does not grant a workspace, calendar access, or a staff role. Those permissions are approved separately.</p><InstallCompass entry /><a className="creator-recovery-link" href="/app/creator-recovery">Recovery access</a></section></main>;
}
...
export function PasswordRecoveryProblem({ supabase, detail }: { supabase: SupabaseClient; detail: string }) {
  ...
  return <main className="production-auth"><section className="production-auth-card production-auth-card--recovery">
    ...
    <a className="preview-entry" href="/app?preview=creator"><span><strong>Explore Compass demos</strong><small>Continue exploring Compass with fictional records while account access is repaired.</small></span><span aria-hidden="true">→</span></a>
