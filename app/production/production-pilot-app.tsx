...
import { WorkspaceHub } from "./workspace-ui";
import { WorkspaceNavigation, WorkspaceBack, usePathwayView, useSessionDraft } from "./workspace-navigation";
...
import { CreatorControls, PrincipalBadge } from "./creator-controls";
...
type AdminView = "home" | "people" | "sessions" | "surveys" | "configuration" | "creator";
...
function AppHeader({ context, role, onRole, onSignOut }: { context: AuthorizationContext; role: PilotRole; onRole: (role: PilotRole) => void; onSignOut: () => void }) {
  return <header className="production-header"><a className="production-brand" href="/app"><img src={assetUrl("/assets/navigate-pathway-mark.svg")} alt="" /><span>Navigate The Pathway</span></a><div className="production-account"><span>{context.displayName}</span>{context.principalType ? <PrincipalBadge context={context} /> : null}{context.roles.length > 1 ? <label><span className="sr-only">Dashboard</span><select value={role} onChange={(event) => onRole(event.target.value as PilotRole)}>{context.roles.map((item) => <option key={item} value={item}>{item[0].toUpperCase() + item.slice(1)}</option>)}</select></label> : <span className="role-chip">{role}</span>}<button className="text-button" onClick={onSignOut}>Sign out</button></div></header>;
}
...
function AdminDashboardView({ dashboard, api, context, reload }: { dashboard: AdminDashboard; api: PilotApiClient; context: AuthorizationContext; reload: () => void }) {
  ...
  return <><nav className="production-tabs production-tabs--scroll" aria-label="Administrator dashboard">{navigation.map(([id, label]) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)}>{label}</button>)}</nav>
    ...
    {view === "creator" && context.principalType ? <CreatorControls api={api} context={context} /> : null}</>;
}
...
function Dashboard({ session, supabase }: { session: Session; supabase: SupabaseClient }) {
  ...
  return <div className="production-shell"><AppHeader context={context} role={role} onRole={(next) => { setRole(next); setDashboard(null); }} onSignOut={() => void signOut()} /><main className="production-main"><WorkspaceBack /><div className="production-welcome"><p className="kicker">{role} dashboard</p><h1>{role === "student" ? "Your pathway home." : role === "advisor" ? "Your advising home." : "Your program home."}</h1></div>{message ? <p className="form-message" aria-live="polite">{message}</p> : null}<WorkspaceHub api={api} context={context} activeRole={role}>{dashboard && role === "student" ? <StudentDashboardView dashboard={dashboard as StudentDashboard} api={api} supabase={supabase} userId={context.userId} reload={loadDashboard} /> : dashboard && role === "advisor" ? <AdvisorDashboardView dashboard={dashboard as AdvisorDashboard} api={api} reload={loadDashboard} /> : dashboard && role === "administrator" ? <AdminDashboardView dashboard={dashboard as AdminDashboard} api={api} context={context} reload={loadDashboard} /> : null}</WorkspaceHub></main></div>;
}
