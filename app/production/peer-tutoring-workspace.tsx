...
export function PeerTutoringWorkspace({ api, mode }: { api: PilotApiClient; mode: "peer_tutor" | "tutoring_manager" }) {
  ...
  if (view !== "home") return <section className={`experience-panel tutoring-workspace tutoring-workspace--${mode === "peer_tutor" ? "tutor" : "manager"}`}><button className="workspace-back text-button" onClick={() => setView("home")}>← Tutoring home</button><header className="staff-page-heading"><p className="kicker">{mode === "peer_tutor" ? "Peer Tutor" : "Tutoring Manager"}</p><h1>{title}</h1></header>{message ? <p className="form-message" aria-live="polite">{message}</p> : null}{data.role === "peer_tutor" ? <TutorDetail data={data} view={view} action={action} message={message} /> : <ManagerDetail data={data} view={view} action={action} />}</section>;
