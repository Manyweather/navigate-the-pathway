...
import { StudentAffiliations } from "./student-affiliations";
...
import {
  ...
  type OacaAvailabilitySlot,
} from "./oaca-student-availability";
...
type EncounterRecord = { appointmentId: string; workingNotes: string; studentRecap: string; structuredData: { categories: string[]; interventions: string[]; referrals: string[]; followUp: string[] }; revision: number; updatedAt: string; publishedAt: string | null };
...
const fallbackServices: Service[] = oacaServiceLines.map((service) => ({ id: "", key: service.key, name: service.name, providerRule: service.providerRule, policyStatus: "sandbox_approved", modalities: service.key === "peer_tutoring" ? ["in_person", "teams"] : ["in_person", "phone", "teams", "zoom"], durationMinutes: service.key === "peer_tutoring" ? 60 : 30 }));
const emptyBootstrap: Bootstrap = { services: [], providers: [], appointments: [], encounterRecords: [], assignedAdvisor: null, assignedStudents: [], currentProvider: null, policyDocuments: [], policyRules: [], acknowledgments: [], obligations: [], restrictions: [], tutorCompliance: null, liveScheduling: false, calendarConnected: false, canManageImports: false, canViewAnalytics: false, importBatches: [], analytics: null, canManageOutreach: false, canViewOutreachInsights: false, events: [], campaigns: [], nudges: [], communications: [], forms: [], audienceOptions: { cohorts: [], phases: [], years: [], campuses: [] }, eventNotificationUnreadCount: 0 };
...
function meetingFormatLabel(value: string) {
...
function StudentAvailabilityPicker({
  slots,
  serviceKey,
  providerIds,
  selectedProviderId,
  ...
  const providers = [...new Map(allFiltered.map((slot) => [slot.providerId, slot])).values()];
  const filtered = selectedProviderId && providers.length > 1
    ? allFiltered.filter((slot) => slot.providerId === selectedProviderId)
    : allFiltered;
  const days = groupAvailabilityByDay(filtered);
  const activeDay = days.some((day) => day.key === requestedDay)
    ? requestedDay
    : days[0]?.key || "";
  ...
  return (
    <fieldset className={`student-availability-picker${compact ? " student-availability-picker--compact" : ""}`} data-tutorial-id="student-availability">
      ...
      <div className="availability-source-note">
        ...
        <div>
          <strong>Compass availability</strong>
          <p>These time blocks come from advisor availability entered in Compass. Outlook conflict checking is not connected in this pilot, so your advisor will confirm the request.</p>
        </div>
      </div>
      {providers.length > 1 ? (
        <div className="availability-advisors" aria-label="Available advisors">
          {providers.map((provider) => {
            const count = allFiltered.filter((slot) => slot.providerId === provider.providerId).length;
            return (
              <button
                type="button"
              ...
              >
                <span>{providerInitials(provider.providerName)}</span>
                <span><strong>{provider.providerName}</strong><small>{provider.providerRole} · {count} time{count === 1 ? "" : "s"}</small></span>
              </button>
            );
          })}
        </div>
      ...
      {days.length ? (
        <>
          <div className="availability-days" aria-label="Available days">
            {days.map((item) => (
              <button
                type="button"
                key={item.key}
                className={item.key === activeDay ? "selected" : ""}
                aria-pressed={item.key === activeDay}
                onClick={() => setRequestedDay(item.key)}
              >
                <strong>{item.shortLabel}</strong>
                <small>{item.slots.length} opening{item.slots.length === 1 ? "" : "s"}</small>
              </button>
            ))}
          </div>
          <div className="availability-chart" aria-label={`Available time blocks for ${day?.label || "selected day"}`}>
            <div className="availability-chart__heading">
              <div><span>Time-block view</span><strong>{day?.label}</strong></div>
              <small>{meetingFormatLabel(modality)}</small>
            </div>
            {bands.map((band) => {
              const bandSlots = (day?.slots || []).filter(
                (slot) => availabilityTimeBand(slot.startsAt) === band,
              );
              return (
                <div className="availability-band" key={band}>
                  <span>{band}</span>
                  <div>
                    {bandSlots.map((slot) => (
                      <button
          ...
          {selectedStartsAt ? (
            <p className="availability-selection" role="status">
              <span aria-hidden="true">✓</span>
              <strong>{new Date(selectedStartsAt).toLocaleString(undefined, { weekday: "long", month: "long", day: "numeric", hour: "numeric", minute: "2-digit" })}</strong>
              selected for request
            </p>
          ) : <p className="availability-prompt" role="status">Select one of the available time blocks above.</p>}
        </>
      ) : (
        <div className="availability-empty-state">
          <strong>No published times match this selection.</strong>
          <p>Try another modality or advisor. Additional availability can be added directly in Compass without Outlook.</p>
        </div>
      )}
    </fieldset>
  );
...
function OacaStudent({ api, supabase, context, data, view, setView, reload }: { api: PilotApiClient; supabase: SupabaseClient; context: AuthorizationContext; data: Bootstrap; view: OacaView; setView: (view: OacaView) => void; reload: () => Promise<void> }) {
  ...
  const careerProvider = data.providers.find((provider) => provider.serviceKeys?.includes("career_advising"));
  const availabilityProviderIds = studentVisibleAvailabilityProviderIds({
    serviceKey,
    assignedAcademicAdvisorId: data.assignedAdvisor?.id,
    careerAdvisorId: careerProvider?.id,
    peerTutorIds: peerTutors.map((provider) => provider.id),
  });
  const availabilitySelectedProviderId = serviceKey === "academic_advising"
    ? data.assignedAdvisor?.id || ""
  ...
  const schedule = async (event: React.FormEvent) => {
    ...
    if (!startsAt) { setMessage("Choose one of the available time blocks."); return; }
  ...
  if (view === "schedule") {
    return <section className="experience-panel">
      ...
      <ol className="booking-progress booking-progress--lifecycle" aria-label="Appointment process">
        ...
        <li><span>3</span><strong>Meet</strong><small>Connect in person, by phone, Teams, or Zoom</small></li>
      </ol>
      <form className="experience-form" onSubmit={schedule}>
        ...
        {serviceKey === "academic_advising" && !quickSlotSelected ? <>
          <fieldset>
            ...
            <div className="segmented-control">
              <label><input type="radio" name="academic-visit-type" checked={academicVisitType === "assigned"} onChange={() => { setAcademicVisitType("assigned"); setProviderId(""); setStartsAt(""); setAcademicBlockKey(""); }} /><span>Meet with my assigned advisor</span></label>
              <label><input type="radio" name="academic-visit-type" checked={academicVisitType === "drop_in"} onChange={() => { setAcademicVisitType("drop_in"); setProviderId(""); setStartsAt(""); }} /><span>Academic drop-in</span></label>
            </div>
          </fieldset>
          {academicVisitType === "assigned"
            ...
            : <div className="drop-in-fields">
                ...
                <label><span>Drop-in advisor</span><select required value={providerId} onChange={(event) => { setProviderId(event.target.value); setStartsAt(""); }}><option value="">Choose an academic advisor</option>{academicDropInProviders.map((provider) => <option key={provider.id} value={provider.id}>{provider.displayName}</option>)}</select></label>
                <label>
        ...
        <button className="primary-button" disabled={busy || !startsAt}>{busy ? "Submitting…" : data.liveScheduling ? "Submit request" : "Submit sandbox request"}</button>
  ...
  return <section className="experience-panel">
    ...
    <section className="home-appointment-scheduler" aria-labelledby="home-scheduler-title" data-tutorial-id="student-availability">
      ...
      <div className="home-scheduler-controls">
        ...
        <label><span>Format</span><select value={modality} onChange={(event) => { setModality(event.target.value); setStartsAt(""); }}><option value="in_person">In person</option><option value="teams">Teams</option>{serviceKey !== "peer_tutoring" ? <><option value="zoom">Zoom</option><option value="phone">Phone</option></> : null}</select></label>
...
function OacaStaff({ api, supabase, context, data, mode, reload, view, setView }: { api: PilotApiClient; supabase: SupabaseClient; context: AuthorizationContext; data: Bootstrap; mode: string; reload: () => Promise<void>; view: OacaView; setView: (view: OacaView) => void }) {
  const defaultRecordAppointmentId = data.appointments.find((item) => item.status === "completed")?.id || data.appointments[0]?.id || "";
  const [notes, setNotes] = useState(""); const [recap, setRecap] = useState(""); const [category, setCategory] = useState("academic_planning"); const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false); const [appointmentId, setAppointmentId] = useState(defaultRecordAppointmentId); const [replacement, setReplacement] = useState("");
  ...
  const save = async (publishRecap: boolean) => { if (!selected) { setMessage("Choose an appointment first."); return; } setBusy(true); try { await api.request("/api/oaca/records", { method: "POST", body: { appointmentId: selected.id, workingNotes: notes, studentRecap: recap, structuredData: { categories: [category], interventions: selectedRecord?.structuredData.interventions || [], referrals: selectedRecord?.structuredData.referrals || [], followUp: selectedRecord?.structuredData.followUp || [] }, publishRecap } }); await reload(); setMessage(publishRecap ? "Revision saved and the student action plan was published." : "Protected notes saved as a new audited revision."); } catch (error) { setMessage(error instanceof Error ? error.message : "The encounter record could not be saved."); } finally { setBusy(false); } };
  ...
  if (view === "policies") return <PolicyLibrary data={data} onBack={() => setView("home")} />;
  if (view === "schedule_student") return <section className="experience-panel"><button className="workspace-back text-button" onClick={() => setView("home")}>← Staff home</button><p className="kicker">Advisor-created appointment</p><h1>Schedule with a student.</h1><p className="policy-intro">Academic advising is limited to assigned students unless an explicit scheduling capability has been granted. Every appointment is labeled advisor-initiated and audited.</p><form className="experience-form" onSubmit={scheduleForStudent}><label><span>Student</span><select required value={studentId} onChange={(event) => { setStudentId(event.target.value); setObligationId(""); }}><option value="">Choose an authorized student</option>{data.assignedStudents.map((student) => <option key={student.id} value={student.id}>{student.displayName}</option>)}</select></label><label><span>Service</span><select required value={serviceId} onChange={(event) => setServiceId(event.target.value)}><option value="">Choose academic or career advising</option>{advisingServices.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label>{studentObligations.length ? <label><span>Link a required milestone (optional)</span><select value={obligationId} onChange={(event) => setObligationId(event.target.value)}><option value="">Additional appointment</option>{studentObligations.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label> : null}<label><span>Purpose</span><input required value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="Reason for the appointment" /></label><label><span>Date and time</span><input type="datetime-local" required value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></label><fieldset><legend>Modality</legend><div className="segmented-control">{["in_person", "phone", "teams", "zoom"].map((item) => <label key={item}><input type="radio" name="advisor-modality" value={item} checked={modality === item} onChange={() => setModality(item)} /><span>{meetingFormatLabel(item)}</span></label>)}</div></fieldset><button className="primary-button" disabled={busy || !data.assignedStudents.length}>{busy ? "Creating…" : "Create appointment"}</button>{!data.assignedStudents.length ? <p>No students are currently available within your assignment or scheduling capability.</p> : null}<p className="form-message" aria-live="polite">{message}</p></form></section>;
  if (view === "records") return <section className="experience-panel">
    <button className="workspace-back text-button" onClick={() => setView("home")}>← Staff home</button>
    <p className="kicker">Visit records</p><h1>Review and continue the advising record.</h1>
    <div className="record-editor">
      <label><span>Session</span><select value={selected?.id || ""} onChange={(event) => setAppointmentId(event.target.value)}><option value="">Choose a session</option>{data.appointments.map((item) => <option key={item.id} value={item.id}>{item.status === "completed" ? "Completed" : item.status.replaceAll("_", " ")} · {item.studentName} · {item.serviceName} · {item.startsAt ? new Date(item.startsAt).toLocaleString() : "time pending"}</option>)}</select></label>
      {selected?.status === "completed" ? <section className="completed-session-summary" aria-label="Completed session summary">
        <div><span className="status-chip status-chip--completed">Completed session</span><h2>{selected.subject || selected.serviceName}</h2><p>{selected.studentName} with {selected.providerName || "assigned advisor"}</p></div>
        <dl><div><dt>Date</dt><dd>{selected.startsAt ? new Date(selected.startsAt).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" }) : "Not recorded"}</dd></div><div><dt>Format</dt><dd>{selected.modality.replaceAll("_", " ")}</dd></div><div><dt>Record</dt><dd>{selectedRecord ? `Revision ${selectedRecord.revision}` : "No notes yet"}</dd></div></dl>
        <p>{selectedRecord ? `Last updated ${new Date(selectedRecord.updatedAt).toLocaleString()}. You can edit or add to the notes below; saving creates another audited revision.` : "Add the first protected note or student recap below."}</p>
      </section> : null}
      {selected && ["pending_approval", "counterproposed"].includes(selected.status) ? <div className="review-card"><h2>Provider decision</h2><div className="workspace-actions"><button className="primary-button" disabled={busy} onClick={() => void decide("confirm")}>Confirm request</button><button className="text-button" disabled={busy} onClick={() => void decide("decline")}>Decline</button></div><label><span>Or propose another time</span><input type="datetime-local" value={replacement} onChange={(event) => setReplacement(event.target.value)} /></label><button className="secondary-button" disabled={busy || !replacement} onClick={() => void decide("counterpropose")}>Send counterproposal</button></div> : null}
      <label><span>Protected staff working notes</span><textarea rows={8} value={notes} onChange={(event) => setNotes(event.target.value)} /><small>Never shown to the student. Access is limited to assigned service staff.</small></label>
      <label><span>Structured category</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="academic_planning">Academic planning</option><option value="career_exploration">Career exploration</option><option value="learning_strategy">Learning strategy</option><option value="referral">Referral</option></select></label>
      <label><span>Student-facing recap and action plan</span><textarea rows={6} value={recap} onChange={(event) => setRecap(event.target.value)} /><small>The student sees only a recap that staff intentionally publish.</small></label>
      <div className="workspace-actions"><button className="secondary-button" disabled={busy || !selected} onClick={() => void save(false)}>{selectedRecord ? "Save note revision" : "Save protected draft"}</button><button className="primary-button" disabled={busy || !selected || !recap.trim()} onClick={() => void save(true)}>{selectedRecord?.publishedAt ? "Update published recap" : "Publish recap to student"}</button></div>
      <p className="form-message" aria-live="polite">{message}</p>
    </div>
  </section>;
  ...
  return <section className="experience-panel">
    ...
    <div className="home-action-grid home-action-grid--experience">
      ...
      <button onClick={() => setView("settings")}><span>⚙</span><strong>Service configuration</strong><small>Policy mapping and live gates</small></button>
      <button onClick={() => setView("policies")}><span>§</span><strong>Policy library</strong><small>Source versions and operational rules</small></button>
    </div>
  </section>;
}
