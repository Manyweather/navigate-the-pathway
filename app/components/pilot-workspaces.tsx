...
import { studentInstrumentCatalog } from "../production/catalog";
...
export function PilotStudentWorkspace() {
  ...
  return <div className="workspace-grid pilot-workspace">
    <section className="workspace-card workspace-card--wide"><p className="kicker">Sessions</p><h2>Attend, reflect, and keep your planning materials together.</h2><p className="workspace-intro">This local demonstration uses fictional sessions and survey shells. It does not submit institutional records.</p></section>
    ...
    <section className="workspace-card workspace-card--wide"><p className="kicker">Evaluation</p><h2>Named survey availability</h2><div className="survey-card-grid">{studentInstrumentCatalog.map((instrument) => { const wave = state.pilot.surveys.waves[0]; const response = state.pilot.surveys.responseSets.find((item) => item.waveId === wave.id && item.completedPlaceholderSteps.includes(instrument.slug)); const available = isSurveyWaveAvailable(wave); return <article className="survey-card" key={instrument.slug}><div><span>{wave.label}</span></div><h3>{instrument.name}</h3><p>{instrument.itemCount} rating items{instrument.openResponseCount ? ` and ${instrument.openResponseCount} open responses` : ""}</p><button className="secondary-button" disabled={!available || response?.status === "complete"} onClick={() => setPilotState((current) => upsertSurveyResponseSet(current, wave.id, [instrument.slug], true))}>{response?.status === "complete" ? "Submitted" : available ? "Open survey shell" : "Not available"}</button></article>; })}</div><p className="workspace-warning">Approved questions and permissions are not loaded in this public prototype. Production survey content remains in the authenticated backend.</p></section>
  </div>;
}
