...
import {
  ...
  aamcLimits,
  aamcMdPhdGuidance,
  aamcOptionalContextPrompts,
  aamcPersonalEssayLenses,
...
function ReflectionWorkspace() {
  const { state, dispatch } = usePrototype();
  const experiences = state.artifacts.filter((item) => item.kind === "experience");
  const storySources = state.artifacts.filter((item) => item.kind === "experience" || item.kind === "reflection");
  ...
  const [activeEssayLens, setActiveEssayLens] = useState<string>(aamcPersonalEssayLenses[0].id);
  const [essayAnswers, setEssayAnswers] = useState<Record<string, string>>({});
  const [contextNote, setContextNote] = useState("");
  const [workingDraft, setWorkingDraft] = useState("");
  const reflectionText = `${happened} ${mattered} ${next}`;
  const currentEssayLens = aamcPersonalEssayLenses.find((item) => item.id === activeEssayLens) ?? aamcPersonalEssayLenses[0];
  const essayAnswerCount = aamcPersonalEssayLenses.filter((item) => essayAnswers[item.id]?.trim()).length;
  const essayMapText = aamcPersonalEssayLenses.map((item) => `${item.label}: ${essayAnswers[item.id]?.trim() || "Not explored yet"}`).join("\n") + (contextNote.trim() ? `\nOptional context: ${contextNote.trim()}` : "");
  const essaySignals = sensitiveSignals(`${essayMapText} ${workingDraft}`);
  useAutosavedDraft("reflection:active", "reflection", { linked, happened, mattered, next });
  useAutosavedDraft("story:active", "story", { theme, sourceIds, activeEssayLens, ...essayAnswers, contextNote, workingDraft });
  ...
  const saveEssayMap = () => { dispatch({ type: "ADD_ARTIFACT", artifact: makeArtifact("story", theme.trim() || "Personal essay map", essayMapText, "Story Studio", { theme, sourceIds: sourceIds.join(","), essayType: "personal_comments", workingDraft, targetCharacters: aamcLimits.personalCommentsCharacters }) }); setTheme(""); setSourceIds([]); setEssayAnswers({}); setContextNote(""); setWorkingDraft(""); setActiveEssayLens(aamcPersonalEssayLenses[0].id); dispatch({ type: "CLEAR_DRAFT", key: "story:active" }); };
  return <div className="workspace-grid"><section className="workspace-card"><div className="visual-sequence"><span>What happened</span><span>Why it mattered</span><span>What changes next</span></div><h2>Build a reflection</h2><Field label="Linked experience"><select value={linked} onChange={(event) => setLinked(event.target.value)}><option value="">None yet</option>{experiences.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></Field><Field label="What happened?"><textarea value={happened} onChange={(event) => setHappened(event.target.value)} /></Field><Field label="Why did it matter?"><textarea value={mattered} onChange={(event) => setMattered(event.target.value)} /></Field><Field label="What changes next?"><textarea value={next} onChange={(event) => setNext(event.target.value)} /></Field><PrivacySignals value={reflectionText} /><button className="primary-button" disabled={!happened.trim() || !mattered.trim() || sensitiveSignals(reflectionText).length > 0} onClick={saveReflection}>Save reflection</button></section><section className="workspace-card workspace-card--wide"><div className="visual-sequence"><span>Motivation</span><span>Evidence</span><span>Perspective</span><span>Direction</span></div><div className="section-count"><div><h2>Personal essay map</h2><p>Explore one lens at a time. You are collecting truthful ingredients, not forcing a polished essay today.</p></div><strong>{essayAnswerCount}/4<small>lenses explored</small></strong></div><div className="essay-lens-picker" role="group" aria-label="Personal essay lenses">{aamcPersonalEssayLenses.map((item) => <button type="button" key={item.id} className={activeEssayLens === item.id ? "active" : ""} aria-pressed={activeEssayLens === item.id} onClick={() => setActiveEssayLens(item.id)}><span>{essayAnswers[item.id]?.trim() ? "✓" : "○"}</span>{item.label}</button>)}</div><div className="essay-prompt-card"><strong>{currentEssayLens.label}</strong><p>{currentEssayLens.prompt}</p><textarea aria-label={currentEssayLens.label} value={essayAnswers[currentEssayLens.id] || ""} onChange={(event) => setEssayAnswers((current) => ({ ...current, [currentEssayLens.id]: event.target.value }))} /></div><div className="field-pair"><Field label="Through-line or theme"><input value={theme} onChange={(event) => setTheme(event.target.value)} placeholder="What connects these ideas?" /></Field><fieldset className="source-picker"><legend>Evidence from your saved work</legend>{storySources.length ? storySources.map((item) => <label key={item.id}><input type="checkbox" checked={sourceIds.includes(item.id)} onChange={(event) => setSourceIds((current) => event.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} />{item.title}</label>) : <p>Save an experience or reflection to connect it here later.</p>}</fieldset></div><details className="guidance-details"><summary>Optional context to consider</summary><p className="guidance-intro">You do not need to disclose hardship. Use this only when context helps readers understand your path.</p><ul className="prompt-list">{aamcOptionalContextPrompts.map((item) => <li key={item}>{item}</li>)}</ul><Field label="Private context note"><textarea value={contextNote} onChange={(event) => setContextNote(event.target.value)} /></Field></details><details className="guidance-details"><summary>Draft in plain text when you are ready</summary><Field label="Personal Comments working draft"><textarea className="essay-draft" maxLength={aamcLimits.personalCommentsCharacters} value={workingDraft} onChange={(event) => setWorkingDraft(event.target.value)} /></Field><p className="field-counter">{workingDraft.length} / {aamcLimits.personalCommentsCharacters} characters</p></details><details className="source-note"><summary>Student voice and final review</summary><ul>{aamcEssayChecks.map((item) => <li key={item}>{item}</li>)}</ul><p>Rosie may support brainstorming, proofreading, or editing. The final submission must reflect your own work and experiences. {aamcGuide.preparationNotice}</p></details><details className="source-note"><summary>Considering a combined MD-PhD program?</summary><ul>{aamcMdPhdGuidance.map((item) => <li key={item}>{item}</li>)}</ul><p>Current guide limits: {aamcLimits.mdPhdCharacters.toLocaleString()} characters for the MD-PhD essay and {aamcLimits.significantResearchCharacters.toLocaleString()} for the Significant Research Experience essay.</p></details><PrivacySignals value={`${essayMapText} ${workingDraft}`} /><button className="primary-button" disabled={!essayAnswerCount || essaySignals.length > 0} onClick={saveEssayMap}>Save essay map</button></section></div>;
}
